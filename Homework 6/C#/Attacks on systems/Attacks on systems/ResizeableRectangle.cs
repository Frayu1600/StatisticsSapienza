...
{
    public enum chartType { PlusMinus, Freq, RelativeFreq, NormalizedFreq, SandP };
    ...
    internal class ResizeableRectangle : PictureBox
    {
        ...
        public ResizeableRectangle(int x, int y, int width, int height, int attacks, int cornerSize, chartType ct, PictureBox pictureBox, int startS = 0, int sStep = 0, int sLevels = 10, int P = 0)
        {
            ...
            this.x = rect.X;
            this.y = (ct == chartType.PlusMinus || ct == chartType.SandP) ? HalfwayYPoint : rect.Bottom;
        ...
        public void DrawChartOnBitmap()
        {
            using (Graphics g = Graphics.FromImage(pictureBox.Image))
            {
                ...
                if (ct == chartType.PlusMinus || ct == chartType.SandP) g.DrawLine(Pens.Red, rect.Left, HalfwayYPoint, rect.Right, HalfwayYPoint);
        ...
        public void CreateHistogram()
        {
            ...
            float minScore = markedSystems.Min();
            float maxScore = markedSystems.Max();

            // calculate the number of rectangles to draw
            int boxes = Math.Max(5, markedSystems.Length/10);
            ...
            float intervalWidth = (maxScore - minScore) / markedSystems.Length; //intervals.Length;

            // add values to every interval accordingly
            //for (int i = 0; i < markedSystems.Length; i++)
            //{
            //double intervalIndex = Math.Floor((markedSystems[i] - minScore) / intervalWidth);
            ...
            int UIRectWidthUnit;
            for (int i = 0; i < markedSystems.Length; i++) //intervals.Length; i++)
            {
                // adjust the max height to avoid overflow
                UIRectWidthUnit = rect.Width / Math.Abs(markedSystems.AsQueryable().Max()) / 2; //Math.Abs(intervals.AsQueryable().Max()) / 2;
                histogramRectY = rect.Y + i * UIRectHeight;
                Rectangle histogramRect = new Rectangle
                (
                    rect.Right - markedSystems[i] * UIRectWidthUnit, //intervals[i] * UIRectWidthUnit,
                    (int)histogramRectY,
                    markedSystems[i] * UIRectWidthUnit, //intervals[i] * UIRectWidthUnit,
                    (int)UIRectHeight
                );
        ...
        public void SimulateAttack(bool defended, Color color, int _SYSTEMS_COUNT)
        {
            ...
            using (Graphics g = Graphics.FromImage(pictureBox.Image))
            {
                ...
                if(attacksPerformed == 1) g.DrawLine(pen, previousx, (ct == chartType.PlusMinus || ct == chartType.SandP) ? HalfwayYPoint : rect.Bottom, x, y);
